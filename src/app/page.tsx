    ...
    <main className="min-h-screen bg-[#0a0a0f] text-white flex flex-col overflow-hidden relative">
      ...
      <nav className="relative z-10 flex items-center justify-between px-8 py-5 border-b border-white/5">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => router.push("/auth")}
            className="text-sm bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 px-4 py-2 rounded-lg transition-all font-medium"
          >
            Get Started Free
          </button>
        </div>
      ...
      <section className="relative z-10 flex flex-col items-center justify-center flex-1 px-4 pt-24 pb-10 text-center">
        ...
        >
          ...
          <p className="text-white/50 text-lg md:text-xl max-w-xl mx-auto mb-10 leading-relaxed">
            Elixir brings the world's best AI models straight into Roblox Studio.
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 justify-center">
            <button
              onClick={() => router.push("/auth")}
              className="w-full sm:w-auto bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-semibold px-8 py-3.5 rounded-xl transition-all text-sm shadow-lg shadow-purple-900/40 hover:shadow-purple-800/50 hover:scale-105"
            >
        ...
        {/* How it works strip */}
        <div
          className={`mt-20 flex flex-col md:flex-row items-center justify-center gap-4 text-sm transition-all duration-1000 delay-200 ${
            mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
          }`}
        >
          {[
            { step: "1", label: "Connect Roblox", icon: "🎮", desc: "Link your account via OAuth2" },
            { step: "2", label: "Install Plugin", icon: "🔌", desc: "Add Elixir to Roblox Studio" },
            { step: "3", label: "Plugin Goes Live", icon: "✅", desc: "Chat unlocks automatically" },
            { step: "4", label: "Build with AI", icon: "💬", desc: "Ask anything inside Studio" },
          ].map((item, i) => (
            <div key={item.step} className="flex items-center gap-4">
              <div className="flex flex-col items-center bg-white/5 border border-white/10 rounded-2xl px-6 py-4 w-44 hover:border-purple-500/30 transition-all hover:bg-white/8">
        ...
        {/* Model cards */}
        <div
          className={`mt-16 grid grid-cols-2 md:grid-cols-4 gap-3 max-w-3xl w-full transition-all duration-1000 delay-300 ${
            mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
          }`}
        >
          {[
            { name: "Gemini 3.0 Flash", tier: "Basic", color: "from-blue-500 to-cyan-500", credits: "0.15 cr/msg" },
            { name: "Step 3.5", tier: "Standard", color: "from-green-500 to-emerald-500", credits: "0.25 cr/msg" },
            { name: "Claude Opus 3.5", tier: "Advanced", color: "from-orange-500 to-amber-500", credits: "0.50 cr/msg" },
            { name: "Claude Sonnet 4.6", tier: "Premium", color: "from-purple-500 to-indigo-500", credits: "0.75 cr/msg" },
          ].map((m) => (
            <div
              key={m.name}
              className="bg-white/5 border border-white/10 rounded-xl p-4 text-left hover:bg-white/8 transition-all hover:border-white/20 hover:scale-105 cursor-default"
            >
              <div className={`text-xs font-semibold bg-gradient-to-r ${m.color} bg-clip-text text-transparent mb-2`}>
                {m.tier}
              </div>
      ...
      <section className="relative z-10 px-8 py-16 max-w-5xl mx-auto w-full">
        <h2 className="text-center text-2xl font-bold mb-10 text-white/90">
          Everything you need, inside Studio
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            {
              icon: "🔐",
              title: "Roblox-Linked Auth",
              desc: "Your account is tied to your Roblox User ID. No email needed — just connect and go.",
            },
            {
              icon: "🔌",
              title: "Plugin Required",
              desc: "Chat only works when your Studio plugin is live. Keeps everything in sync and secure.",
            },
            {
              icon: "⚡",
              title: "Streaming Responses",
              desc: "Answers appear word by word in real time. Fast, fluid, and never laggy.",
            },
            {
              icon: "🧠",
              title: "4 Powerful Models",
              desc: "From quick questions to deep reasoning — pick the right model for the job.",
            },
            {
              icon: "💎",
              title: "Credit System",
              desc: "1 free daily credit included. Buy more or go Premium for the best value.",
            },
            {
              icon: "🎮",
              title: "Built for Builders",
              desc: "Scripting help, game design ideas, Lua debugging — Elixir knows Roblox.",
            },
          ].map((f) => (
            <div
              key={f.title}
              className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:border-purple-500/30 transition-all hover:bg-white/8"
            >
      ...
      <section className="relative z-10 px-8 pb-20 max-w-3xl mx-auto w-full">
        <div className="bg-gradient-to-r from-purple-600/20 to-indigo-600/20 border border-purple-500/30 rounded-2xl p-10 text-center">
          ...
          <p className="text-white/50 text-sm mb-6">
            Connect your Roblox account, install the plugin, and start chatting in under 2 minutes.
          </p>
          <button
            onClick={() => router.push("/auth")}
